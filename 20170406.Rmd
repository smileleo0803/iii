...
# %in%用來檢查變數是否有存在之中，結果為布林值
#要讀取資料框架的“欄位名” (column name), 可以用 names() 函式.
names(churnTrain) %in% c("state", "area_code", "account_length")
!names(churnTrain) %in% c("state", "area_code", "account_length")

#選擇建模變數
variable.list = !names(churnTrain) %in% c('state','area_code','account_length')
#將'state','area_code','account_length'以外的變數放入variable.list
churnTrain=churnTrain[,variable.list] #variable.list→布林向量當條件
